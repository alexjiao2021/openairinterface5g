...
def load_profile(path):
    ...
    return start_unix_s, rows


def initial_ue_hints(rows):
    offset_s = 0.0
    ...
    return offset_s, rows[index][2], drift_us_per_s
...
def main():
    ...
    try:
        _, rows = load_profile(args.profile)
        offset_s, initial_fo, time_drift = initial_ue_hints(rows)
