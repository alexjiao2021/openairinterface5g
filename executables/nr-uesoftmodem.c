...
int main(int argc, char **argv)
{
  ...
  for (int cell_id = 0; cell_id < nrue_get_cell_count(); cell_id++) {
    ...
    if (IS_SA_MODE(get_softmodem_params()) || get_softmodem_params()->sl_mode) {
      ...
      init_timeshift_rotation(cell_fp->ofdm_symbol_size,
                              cell_fp->N_RB_DL * NR_NB_SC_PER_RB,
                              cell_fp->nb_prefix_samples,
                              cell_fp->ofdm_offset_divisor,
                              cell_fp->timeshift_symbol_rotation);
    }
  }
